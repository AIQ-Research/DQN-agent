...
import aiq_ale.aiq_ale
import aiq_experiment
import aiq_agent
import aiq_network
...
def launch(args, defaults, description):
    ...
    if parameters.rom.endswith('.db'):
        rom = parameters.rom
    else:
        rom = "%s.db" % parameters.rom
    ...
    ale = aiq_ale.aiq_ale.FxLocalALE(full_rom_path, 64, [1, 5, 15, 30, 60, 120], rng)

    num_actions = len(ale.getMinimalActionSet())

    if parameters.nn_file is None:
        network = aiq_network.AIQLearner(defaults.RESIZED_WIDTH,
    ...
    agent = aiq_agent.NeuralAgent(network,
    ...
    experiment = aiq_experiment.AIQExperiment(ale, agent,
